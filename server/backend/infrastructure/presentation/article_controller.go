...
func (a *ArticleController) GetRandTreeArticles(ctx *gin.Context){
	articles, err := a.repo.GetRandThreeArticles()
	if err != nil{
		ctx.JSON(http.StatusInternalServerError, gin.H{"error":"error receiving articles"})
		return
	}
...
func (a *ArticleController) ArticlePagination(ctx *gin.Context){
	...
	articles, err := a.repo.GetArticlePaginated(input.Page, input.Limit)
	if err != nil{
		ctx.JSON(http.StatusInternalServerError, gin.H{"error":"error receiving articles"})
		return
	}
